...
rec2_plottable = np.reshape(recon,(70000,28,28))


#%%Applying ICA

# # # Give the amount of components wanted and apply ICA:
ica = FastICA(n_components=182,random_state=0, max_iter = 500)
ica.fit(train_img2)
train_imgica = ica.transform(train_img2)
test_imgica = ica.transform(test_img2)


train_transformed = ica.fit_transform(train_img2)
teat_transformed = ica.fit_transform(test_img2)
...
for i in range(100):
    if labels[i]=='6':
        ...
        print('PCA-PPCA:', sum(sum(abs(rec2_plottable[i]-recpca_plottable[i]))))
        plt.subplot(2,2,1)
        plt.title(labels[i])
