...
if (not os.path.isfile("./Models_test/VAE-{}-{}_0.pth".format(latent_dimensions, l1))) or retrain == "y":
    ...
    loss_t_array = np.zeros(num_epochs)
    loss_v_array = np.zeros(num_epochs)
    ...
    for epoch in range(num_epochs):
        ...
        loss_t_array[epoch] = train_loss
        loss_v_array[epoch] = val_loss

    torch.save(VAE.state_dict(), "./Models_test/VAE-4dim-2250-lamda=0.1-250_epochs.pth")

    plt.figure(figsize = (10,8))
    plt.plot(loss_v_array, '.', label = "val loss")
    plt.plot(loss_t_array, '.', label = "training loss" )
    plt.xlabel("Epochs")
    plt.ylabel("Total loss")
    plt.legend()
    plt.grid()
    plt.savefig("Figures/Loss_250_epochs_2250_lamda-0.1.png")
    plt.show()
    ...
    np.savetxt("Loss_txt/Recon_loss-4dim-2250-lamda=0.1-250_epochs.txt", np.transpose(np.array([lamda, Recon_loss_t, Recon_loss_v, KL_loss_t, KL_loss_v])), fmt = "%1.6f", delimiter = ", ")
...
VAE.load_state_dict(torch.load("./Models_test/VAE-4dim-2250-lamda=0.1-250_epochs.pth"))
